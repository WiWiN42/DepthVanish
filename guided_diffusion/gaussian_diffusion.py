...
def elastic_beta_schedule(num_diffusion):
    beta_start, beta_end = 0.0001, 0.02
    betas = [beta_start]

    # for t in range(num_diffusion):
    #     beta_t = 
...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        return (
            ...
            + _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t_idx, x_start.shape)
            * noise
        )

    def q_sample_mid(self, x_N, N, noise=None):
        """
        Sample from q(x_T | x_N).
        """
        assert isinstance(N, int), 'N must be an int'

        B = x_N.size()[0]
        sqrt_alphas_NT_prod = np.sqrt(np.prod(self.alphas[N:])).repeat(B)
        sqrt_one_minus_alphas_NT_prod = np.sqrt(
            1 - np.prod(self.alphas[N:])
        ).repeat(B)
        
        def expand(res):
            res = torch.from_numpy(res).to(device=x_N.device).float()
            while len(res.shape) < len(x_N.size()):
                res = res[..., None]
            return res.expand(x_N.shape)

        if noise is None:
            noise = torch.randn_like(x_N)
        assert noise.shape == x_N.shape        
        return (
            expand(sqrt_alphas_NT_prod) * x_N 
            + expand(sqrt_one_minus_alphas_NT_prod) * noise
        )
    ...
    def p_sample_loop(
        ...
        if conf.ALGO == 'repaint':
            sample_func = self.p_sample_loop_progressive_tt
        else:
            sample_func = self.p_sample_loop_progressive
    ...
    def p_sample_loop_progressive(
        ...
        if start_time_steps is None:
            ...
            indices = list(range(start_time_steps))[::-1]
        ...
        for i in indices:
            t = torch.tensor([i] * shape[0], device=device) # pylint: disable=not-callable
            with torch.no_grad():
                out = self.p_sample(
                    model,
                    img,
                ...
                #     conf.SAMPLE_DEST,
                #     mode='test_difface_style_base/{}diff'.format(start_time_steps),
                #     shape=(1024, 768),
                #     name_prefix='{}_step_{}'.format(save_prefix, i),
    ...
    def p_sample_loop_progressive_tt(
        self,
        model,
        shape,
        noise=None,
        ...
        for t_last, t_cur in time_pairs:
            ...
            if t_cur < t_last:  # reverse
                with torch.no_grad():
                    ...
                    #     conf.TEST_SAMPLE_INTERVAL.index,
                    #     conf.SAMPLE_DEST,
                    #     mode='test_ddnm_stylev0_repaint_grid/{}diff'.format(start_time_steps),
                    #     shape=(1024, 768),
                ...
                image_after_step = self.undo(
                    ...
                    est_x_0=out['pred_xstart'], t=t_last_t+t_shift, 
                    debug=False)
                pred_xstart = out["pred_xstart"]
    ...
    def p_sample_tt(
        ...
        if pred_xstart is not None:
            ...
            noised_gt = gt_part + noise_part

            x = (1-mask) * noised_gt + mask * x

        out = self.p_mean_variance_tt(
            model,
            x,
            t,
            clip_denoised=clip_denoised,
        ...
        result = {
            "sample": sample,
            "pred_xstart": out["pred_xstart"],
            "img": model_kwargs.get('img')
        }

        return result

    def p_mean_variance_tt(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
    ...
    def _check_times(self, times, t_0, T_sampling):
        # Check end
