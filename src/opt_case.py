...
parser.add_argument("--exp", type=str, default="test", help="the exp to run")
...
parser.add_argument("--unit_norm", type=float, default=0.5, help="the range to constraint the unit's value")
...
for rnd in range(args.round):
    ...
    loss.backward()
    unit_optimizer.step()
    patch_manager._unit.data = lp_projection(patch_manager._unit.data, 0.5, 'inf')
