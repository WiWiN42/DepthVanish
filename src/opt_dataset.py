...
from utils.tool import save_img, save_depth, save_perturb, load_image, lp_projection, round_numbers, read_paths
...
parser.add_argument("--img_left_file", type=str, default="/home/yxing/projects/stereo_PhysicalAttack/data/kitti_scene_flow_val_image0.txt", help="the file storing left images")
parser.add_argument("--img_right_file", type=str, default="/home/yxing/projects/stereo_PhysicalAttack/data/kitti_scene_flow_val_image1.txt", help="the file storing right images")
...
for img_left_file, img_right_file in zip(left_images, right_images):
    ...
    logger.info("Start optimization for {}".format(img_left_file))
    ...
    unit_size = (round(patchL_size[0]/args.n_ytile), round(patchL_size[1]/args.n_xtile))
    logger.debug("The rounded patch specification:")
    logger.debug("pos_left --> %s", str(pos_left))
    logger.debug("pos_right --> %s", str(pos_right))
    logger.debug("patch_size --> %s", str(patch_size))
    logger.debug("unit_size --> %s", str(unit_size))
    ...
    for rnd in range(args.round):
        ...
        img_left = load_image(img_left_file, expand=True)
        img_right = load_image(img_right_file, expand=True)
        # normalization
        ...
        opt_board = patch_manager.assemble_opt_board()
        ...
        loss = regional_mean_square_error(perturb_disp, tl_con=pos_left[0], board_size=patch_size) + args.alpha*entropy_loss(opt_board) + args.gamma*tv_loss(opt_board)
        ...
        loss.backward()
        unit_optimizer.step()
        patch_manager._unit.data = lp_projection(patch_manager._unit.data, args.unit_norm, 'inf')
        ...
        logger.debug('{:<10}  {:>10}  {:>10}  {:>10}  {:>10}'.format('', 'D1-Error ', '+/-', 'EPE', '+/-'))
        logger.debug('{:<10}  {:>10.4f}  {:>10.4f}  {:>10.4f}  {:>10.4f}'.format(
            '',
            d1_error,
            0,
            epe,
        ...
        if rnd % args.n_checkpoint == 0:
            logger.info("Save the Progress of the Optimization: %s", save_dir)

            # save the perturbed stereo images
            save_img(
                imgL,
                os.path.join(img_dir, img_identifier+'_round_{:03d}_left.jpg'.format(rnd))
            )
            save_img(
                imgR,
                os.path.join(img_dir, img_identifier+'_round_{:03d}_right.jpg'.format(rnd))
            )

            # save the mask unit
            save_perturb(
                patch_manager._unit,
                os.path.join(unit_dir, img_identifier+'_round_{:03d}.jpg'.format(rnd))
            )

            # save the assembled board
            save_perturb(
                opt_board,
                os.path.join(patch_dir, img_identifier+'_round_{:03d}.jpg'.format(rnd))
            )

            # save the perturbed depth
            save_depth(
                perturb_disp,
                os.path.join(depth_dir, img_identifier+'_round_{:03d}_perturb.jpg'.format(rnd)),
                # max_disp=150,
                cm=cv2.COLORMAP_JET,
                invert=True
            )

        logger.debug("Optimization Done: loss {:05f}\n".format(loss))

    logger.info('The best result (D1-error Anchored):')
    logger.info('{:<10}  {:>10}  {:>10}  {:>10}  {:>10}'.format('', 'D1-Error ', '+/-', 'EPE', '+/-'))
    logger.info('{:<10}  {:>10.4f}  {:>10.4f}  {:>10.4f}  {:>10.4f}\n\n'.format(
        '',
        best_result['d1'],
        0,
        best_result['epe'],
    ...
    all_d1_result.append(best_result['d1'])
    all_epe_result.append(best_result['epe'])
logger.info('=======================================')
logger.info('The overall results are:')
logger.info('{:<10}  {:>10}  {:>10}  {:>10}  {:>10}'.format('', 'D1-Error ', '+/-', 'EPE', '+/-'))
